...
<html lang="fr">
        ...
        <nav aria-label="Menu principal">
            <ul class="nav-list">
                <li><a href="cr_new.php"> Saisit de Rapport</a></li>
                 <?php if ($role === 'admin' || $role === 'administrateur'): ?><!-- les admins-->
                <li><a href="cr_list.php"> Rapport</a></li>
                  <?php endif?>
                
                  <?php if ($role === 'responsable' || $role === 'responsable'): ?><!-- les resp-->
                <li><a href="cr_list.php"> Rapport</a></li> 
                    <?php endif?>
                     <?php if ($role === 'delegue' || $role === 'delegue'): ?><!--les delegue-->
                <li><a href="cr_list.php"> Rapport</a></li> 
                    <?php endif?>
    ...
    <main class="content">
        <section class="h33rd welcome">
            ...
            <div >Rôle : <strong    class="role"><?= $role ?></strong>
            </div>
            <div> Bienvenue sur Galaxy Swiss Bourdin</p></div>
            </div>
            <div
        ...
        <section class="h33rd">
            ...
            <?php if ($role === 'admin' || $role === 'administrateur'): ?><!--que les admins-->
        ...
        <h3>Informations</h3>
        <!--information-->
        <?php if (in_array($role_raw,['responsable','admin','administrateur'])): ?><!--acces-->
        <div style="display:flex;gap:12px;align-items:center;margin-top:10px;margin-bottom:12px">
            <div style="flex:0 0 220px;padding:12px;border-radius:8px;background:#eef9ff;border:1px solid #d6ecff">
                <div
                 style="font-size:13px;color:#666">Rapports ce mois</div>
                <div
                 style="font-size:22px;font-weight:700;color:#3498db">
                 <?=htmlspecialchars($pctThis)?>%</div>
                <div
                 style="font-size:12px;color:#666">
                 <?=htmlspecialchars($thisMonth)?> / <?=htmlspecialchars($total)?> total</div>
            </div>
            <div
             style="flex:0 0 220px;padding:12px;border-radius:8px;background:#fff7e6;border:1px solid #ffe7c6">
                <div 
                style="font-size:13px;color:#666">Rapports mois précédent</div>
                <div
                 style="font-size:22px;font-weight:700;color:#f39c12">
                 <?=htmlspecialchars($pctLast)?>%</div>
                <div 
                style="font-size:12px;color:#666">
                <?=htmlspecialchars($lastMonth)?> / <?=htmlspecialchars($total)?>total</div>
            </div>
        </div>
        <?php endif?>
        </section>
...
<head>
    ...
    <style>
        *{box-sizing:border-box}
        ...
        .role{color:#e74c3c;margin-top:6px;font-size:0.95rem}
